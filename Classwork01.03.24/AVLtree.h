...
public:
    ...
    {
        if (root != nullptr)return iterator(root->minimum());
        return end();
    }
    ...
    {
        if (root != nullptr)return reverse_iterator(root->maximum());
        return rend();
    }
    ...
    do 
    {
        ...
        switch (now->balance)
        {
            case 2:
                ...
                else now=leftRotate(now);
                break;
            case -2:
                if(now->left->balance==1) now=bigRightRotate(now);
                else now=rightRotate(now);
                break;
        }
    }while(now!=root and now->balance!=0);
}
...
void AVLTree<T>::erase(const typename AVLTree<T>::iterator& pos)
{
    ...
    else
    {
        ...
        {
            notBalanced=alt;
            if(toDelete->left==alt)notBalanced->balance+=1;
            else notBalanced->balance-=1;
        }
        else
        {
    ...
    while (notBalanced)
    {
        ...
        {
            case 2:
                if(notBalanced->right->balance==-1) notBalanced=bigLeftRotate(notBalanced);
                else notBalanced=leftRotate(notBalanced);
                break;
            case -2:
                if(notBalanced->left->balance==1)notBalanced=bigRightRotate(notBalanced);
                else notBalanced=rightRotate(notBalanced);
                break;
        }
        if((notBalanced->balance==1)or(notBalanced->balance==-1)) break;
        if (notBalanced->parent)
        {
            if(notBalanced->parent->left==notBalanced)notBalanced->parent->balance+=1;
            else notBalanced->parent->balance-=1;
        }
